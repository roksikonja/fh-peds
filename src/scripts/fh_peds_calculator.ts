...
export function setupFhpedsCalculator(): void {
  ...
  // matching select is 'bmi_unit'; for the lipid fields it's
      ...
      .forEach((el) => {
        ...
        if (el instanceof HTMLInputElement && el.hasAttribute('aria-describedby')) {
          if (isInvalid) {
            ...
            // currently-selected unit, surface an advisory (amber)
            // warning — without setting the red border or blocking the
